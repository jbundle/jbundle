        ...
        if (m_recHeader != null)
            if ((m_recHeader.getEditMode() == DBConstants.EDIT_ADD) || (m_recHeader.getEditMode() == DBConstants.EDIT_NONE))
                if (this.getProperty(DBParams.HEADER_OBJECT_ID) == null)
                {
                    ...
                    {
                        m_recHeader.getCounterField().setValue(0);
                        m_recHeader.getCounterField().setDefault(m_recHeader.getCounterField().getData()); // This will guarantee that the sub record's parent field is non-null
                        try {
                            m_recHeader.addNew();
