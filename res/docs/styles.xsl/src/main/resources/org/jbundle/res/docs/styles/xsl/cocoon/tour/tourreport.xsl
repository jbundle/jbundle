				...
				<xsl:element name="tr">
					...
					<td class="button">
						<xsl:call-template name="button_link">
							...
							<xsl:with-param name="link">/app?applet=org.jbundle.thin.base.screen.BaseApplet&amp;userid=<xsl:value-of select="//full-screen/params/userid"/>&amp;webStart=thin&amp;screen=com.jbundle.thin.app.booking.entry.tourgeekScreen&amp;objectID=<xsl:value-of select="$record/@id"/>&amp;productType=Tour</xsl:with-param>
