	...
	<xsl:template match="detail/file">
		<table border="0" cellpadding="5" width="100%" class="gridscreen" id="gridscreen">
			...
			<tr>
				...
				<th><xsl:value-of select="//form//detail[attribute::name=$DetailName]/*[attribute::ref='Tour.Description']/xfm:caption"/></th>
				<th><xsl:value-of select="//form//detail[attribute::name=$DetailName]/*[attribute::ref='Tour.DepartureDate']/xfm:caption"/></th>
			</tr>

			<xsl:for-each select="*">
			<xsl:variable name="record" select="." />

			  <xsl:if test="$record/*[name()='Inventory.Available']>0">
				<xsl:element name="tr">
					...
					<td class="button">
						<xsl:call-template name="button_link">
							...
							<xsl:with-param name="link">/app?applet=org.jbundle.thin.base.screen.BaseApplet&amp;userid=<xsl:value-of select="//full-screen/params/userid"/>&amp;webStart=thin&amp;screen=com.jbundle.thin.app.booking.entry.tourgeekScreen&amp;objectID=<xsl:value-of select='$record/TourHeader.ID'/>&amp;date=<xsl:value-of select='$record/Tour.DepartureDate'/>&amp;productType=Tour</xsl:with-param>
							<!-- xsl:with-param name="link">/app?applet=org.jbundle.thin.base.screen.BaseApplet&amp;userid=<xsl:value-of select="//full-screen/params/userid"/>&amp;webStart=thin&amp;screen=com.jbundle.thin.app.booking.entry.tourgeekScreen&amp;TourID=<xsl:value-of select="$record/@id"/></xsl:with-param -->
					...
					<td class="button">
						...
						<td><xsl:value-of select="$record/*[name()='Tour.Description']"/></td>
						<td><xsl:value-of select="$record/*[name()='Tour.DepartureDate']"/></td>
