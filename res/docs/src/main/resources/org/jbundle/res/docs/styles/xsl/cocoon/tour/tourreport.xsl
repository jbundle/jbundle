				...
				<xsl:element name="tr">
					...
					<td class="button">
						<xsl:call-template name="button_link">
							...
							<xsl:with-param name="link">/tourapp?applet=org.jbundle.thin.base.screen.BaseApplet&amp;userid=<xsl:value-of select="//full-screen/params/userid"/>&amp;webStart=thin&amp;screen=com.tourapp.thin.app.booking.entry.TourAppScreen&amp;objectID=<xsl:value-of select="$record/@id"/>&amp;productType=Tour</xsl:with-param>
